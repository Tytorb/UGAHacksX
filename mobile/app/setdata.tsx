...
import { createFile } from '@/constants/queries';
...
function setdata(props: {}) {
  ...
  const submitFn = async () => {
    ...
    createFile(textName, textClue, new Date().toLocaleDateString());
    router.push('/');
  };
...
=======
    <View className="bg-white p-8 h-full pt-48">
      <Text className="text-2xl font-medium">
        Help this band make it by moving them one step closer to their
        destination! Remember to leave a clue.
>>>>>>> 9fcd4e0abf2084a8062cc703ec7f89dcdf817126
      </Text>
      <View className="my-8 items-center justify-center align-center bg-white gap-4">
        <TextInput
          className="flex-col p-3 rounded-xl items-center justify-center bg-gray-100 w-full"
          style={{ padding: 16 }}
          value={textName}
        ...
        />
        <Text className="flex-col p-3 rounded-xl items-center justify-center bg-transparent">
          Date Hidden:{new Date().toLocaleDateString()}
        </Text>
      </View>
      <Pressable
        className="flex-col p-3 rounded-xl items-center justify-center bg-blue-600"
        onPress={submitFn}
        accessibilityLabel="Submit data"
      >
